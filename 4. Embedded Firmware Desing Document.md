...
### 3.2 ESP32 #2 — Mano Robótica (Receptor)

```mermaid
stateDiagram-v2
    [*] --> INIT

    INIT --> STANDBY : Init OK\n(nvs, wifi, espnow,\nbt, gpio_servos,\ngpio_buttons)
    INIT --> ERROR_INIT : Fallo de\ninicialización

    state STANDBY {
        [*] --> ESPERANDO_MODO
        ESPERANDO_MODO --> MODO_GUANTE : Botón GUANTE\npresionado
        ESPERANDO_MODO --> MODO_APP : Botón APP\npresionado
    }

    state MODO_GUANTE {
        [*] --> ESCUCHANDO_ESPNOW
        ESCUCHANDO_ESPNOW --> PROCESANDO_PAQUETE : glove_packet_t\nrecibido
        PROCESANDO_PAQUETE --> ACTUALIZANDO_SERVOS_G : Mapeo\nfinger → ángulo
        ACTUALIZANDO_SERVOS_G --> ESCUCHANDO_ESPNOW : servo_pw[]\nactualizado
        ESCUCHANDO_ESPNOW --> ERROR_ESPNOW_RX : Timeout\n(sin datos > 2s)
        ERROR_ESPNOW_RX --> ESCUCHANDO_ESPNOW : Log WARN\nServos congelan posición
    }

    state MODO_APP {
        [*] --> ESPERANDO_BT
        ESPERANDO_BT --> CONECTADO_BT : SPP SRV_OPEN
        CONECTADO_BT --> PROCESANDO_CMD : DATA_IND\n(letra + ángulo)
        PROCESANDO_CMD --> ACTUALIZANDO_SERVOS_A : procesar_byte()\nángulo válido
        ACTUALIZANDO_SERVOS_A --> CONECTADO_BT : servo_pw[]\nactualizado
        CONECTADO_BT --> ESPERANDO_BT : SPP CLOSE\n(desconexión)
        PROCESANDO_CMD --> ERROR_CMD : Comando\ninválido
        ERROR_CMD --> CONECTADO_BT : Log WARN +\nIgnorar byte
    }

    MODO_GUANTE --> STANDBY : Botón APP\npresionado\n(modo exclusivo)
    MODO_APP --> STANDBY : Botón GUANTE\npresionado\n(modo exclusivo)

    state servo_pwm_task {
        [*] --> GENERANDO_PWM
        GENERANDO_PWM --> GENERANDO_PWM : Loop continuo\nCore 1 / 50Hz
    }
