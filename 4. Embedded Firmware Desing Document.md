...
stateDiagram-v2
    ...
    INIT --> OPERACION : Init OK\n(nvs, gpio, wifi,\nespnow, adc, i2c, lcd)
    INIT --> ERROR_INIT : Fallo de\ninicialización

    state OPERACION {
        [*] --> LEYENDO_SENSORES
        LEYENDO_SENSORES --> EMPAQUETANDO : ADC + MPU6050\nleídos OK
        LEYENDO_SENSORES --> ERROR_SENSOR : Timeout / valor\nfuera de rango
        EMPAQUETANDO --> ENVIANDO_ESPNOW : glove_packet_t listo
        ENVIANDO_ESPNOW --> ACTUALIZANDO_LCD : Envío OK\n+ led_blink()
        ENVIANDO_ESPNOW --> ERROR_ESPNOW : Fallo de envío
        ACTUALIZANDO_LCD --> LEYENDO_SENSORES : Ciclo 20ms
        ERROR_SENSOR --> LEYENDO_SENSORES : Log WARN +\nvalor por defecto
        ERROR_ESPNOW --> LEYENDO_SENSORES : Log ERROR +\nreintentar
    }

    OPERACION --> CALIBRACION : Botón GPIO 18\npresionado

    state CALIBRACION {
        [*] --> MOSTRANDO_CALIBRANDO
        MOSTRANDO_CALIBRANDO --> GUARDANDO_MIN : Primera\npresión
        GUARDANDO_MIN --> GUARDANDO_MAX : Segunda\npresión
        GUARDANDO_MAX --> MOSTRANDO_OK : min[] y max[]\nguardados en NVS
        MOSTRANDO_OK --> [*]
    }

    CALIBRACION --> OPERACION : Calibración\ncompletada
